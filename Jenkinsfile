        ...
        stage('Checkout GIT repository') {
            steps {     
              script {
                ...
                credentialsId: 'd24f2487-0e5d-4f24-8b03-407b62048095',
                url: 'https://github.com/Teomazivila/Chellange101.git'
              }
            }
        }
        ...
        stage('Start new container using latest image and create user') {
            steps {     
              script {
                ...
                def mysqlPasswd = "${params.MYSQL_PASSWORD}"
                sh """
                echo ${mysqlPasswd}
                echo "${mysqlPasswd}"
                docker run -itd --name ${containerName} --rm -e MYSQL_ROOT_PASSWORD=${mysqlPasswd} -p $params.MYSQL_PORT:3306 $params.ENVIRONMENT_NAME:latest
                """

                sh """
                docker exec ${containerName} /bin/bash -c 'mysql --user="root" --password=${mysqlPasswd} < /scripts/create_developer.sql'
                """
